...
void initEnemy(Enemy* e, int type, double x, double y, int health) // сохран€ем тип и координаты врага выбираем логику поведени€
{
    e->x = x;
    e->y = y;
    //e->x = x;
...
void spawnEnemies(Enemy* enemies, int numEnemies, int numOfWave) // непосредственно выбираем тип врага, его начальный спавн и инициализируем
{
    switch (numOfWave)
    {
        ...
        {
            int type = 0;
            //int type = rand() % 3;  // —лучайный выбор типа врага который бежит или стрелка (пока так)
            int health = 30;
